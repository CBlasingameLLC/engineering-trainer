...
const COST_LABEL: Record<string, { text: string; chip: string }> = {
  free: { text: 'Free', chip: 'bg-emerald-100 text-emerald-800' },
  'free-for-students': { text: 'Free for students', chip: 'bg-sky-100 text-sky-800' },
  'discounted-for-students': { text: 'Student discount', chip: 'bg-amber-100 text-amber-800' },
  paid: { text: 'Paid', chip: 'bg-slate-200 text-slate-700' },
};
...
export function Credentials(): React.ReactElement {
  ...
  if (!content) return <div className="grid h-full place-items-center text-sm text-slate-400">Loading…</div>;

  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      <header className="flex items-baseline gap-4">
        <h1 className="text-2xl font-semibold text-slate-900">Credentials worth your time</h1>
      ...
      {top.length > 0 && (
        <section className="mt-8">
          <h2 className="text-sm font-semibold text-slate-900">Start with these</h2>
      ...
      <section className="mt-10">
        <h2 className="text-sm font-semibold text-slate-900">Everything else, ranked</h2>
        <p className="mt-1 text-sm text-slate-600">
  ...
  return (
    <article className={`card p-4 ${highlight ? 'border-slate-400' : ''}`}>
      <div className="flex flex-wrap items-baseline gap-2">
        {rank !== undefined && <span className="font-mono text-xs text-slate-400">{rank}</span>}
        <a href={c.url} target="_blank" rel="noreferrer" className="text-sm font-semibold text-slate-900 underline decoration-slate-300">
          {c.name}
        </a>
        ...
        <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600">{KIND_LABEL[c.kind] ?? c.kind}</span>
        ...
        {ranked.overBudget && (
          <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs text-red-800">Over your hour budget</span>
        )}
        <span className="ml-auto text-xs tabular-nums text-slate-500">{c.effortHours} h</span>
      </div>

      <p className="mt-2 text-sm leading-relaxed text-slate-700">{c.why}</p>
      ...
      <p className="mt-2 border-l-2 border-amber-300 pl-3 text-sm leading-relaxed text-slate-600">{c.caveat}</p>
      {c.cost.note && <p className="mt-2 text-xs text-slate-500">{c.cost.note}</p>}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4 border-t border-slate-100 pt-3">
        ...
        <span className="text-xs italic text-slate-500">{ranked.rationale}</span>
        <span className="ml-auto text-xs text-slate-400">checked {c.checkedOn}</span>
      </div>
    </article>
  );
}
